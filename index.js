...
function adminAuthorized(bodyOrPassword, env) {
  ...
  const supplied = typeof bodyOrPassword === "string"
    ? bodyOrPassword
    : bodyOrPassword?.password;
...
async function handleWeeklyListens(request, env) {
  ...
  try {
    ...
    // Keep only the 500 most-listened videos for the current week.
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === "/api/weekly-listens" || url.pathname === "/api/weekly-listens/") {
      return handleWeeklyListens(request, env);
    }
